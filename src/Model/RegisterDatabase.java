...
// Class Register digunakan untuk menyimpan data user yang akan melakukan registrasi
class Register {
    ...
    public void set_password(String password) {
        this.password = password; // Instansiasi property password dengan parameter password
    }
}
...
public class RegisterDatabase {
    public void sql_register() {
        ...
        try {
            // Mencoba mengkoneksikan ke database celengan dengan username root dan password
            // kosong (tidak ada password)
            connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/celengan", "root", "");
        } catch (SQLException e) { // Menangkap error jika gagal mengkoneksikan ke database
            // Menampilkan pesan error ke console
            System.out.println("Connection failed: " + e.getMessage());
        } finally { // Menjalankan kode di dalam finally setelah try-catch selesai dijalankan
            ...
            if (connection != null) {
                ...
                try {
                    java.sql.Statement statement = connection.createStatement(); // Membuat objek statement
                    // Menjalankan query dan menyimpan hasilnya ke dalam variabel result
                    ResultSet result = statement.executeQuery(sql);
                    System.out.println("Connection closed." + result); // Menampilkan pesan ke console
